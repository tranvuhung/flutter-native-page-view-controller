...
class NativePageViewController {
  ...
  static void show(int pageCount, ContentLoader loader,
      {NativePageViewControllerTransitionStyle transitionStyle =
          NativePageViewControllerTransitionStyle.none,
  ...
  static Future<Future<T?>> load<T>(int pageIndex) async {
    return _channel.invokeMethod<T>('load', pageIndex);
  }

  static int? getPageIndex(String routeString) {
    var match =
        RegExp(pageRouteName + r"(?:\?(\d+))?").matchAsPrefix(routeString);
  ...
  static Widget? getInitialWidget(PageWidgetBuilder pageBuilder) {
    int? pageIndex =
        NativePageViewController.getPageIndex(window.defaultRouteName);
